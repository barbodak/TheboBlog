namespace snake{
    ...
    function draw(x : number, y : number, color : string, type : string = "rect") {
        ctx.beginPath();
        if (type == "rect")
    ...
    function getSnakeSegmentColors(length: number, saturation: number = 100, lightness: number = 50): string[] {
        const colors: string[] = [];
        const colorStep: number = (100) / length;  // Full range of hues from 0 to 360 degrees
        const base : number = 300;  // Full range of hues from 0 to 360 degrees

        for (let i = 0; i < length; i++) {
            // Calculate the hue for each segment
            let hue: number = (base + i * colorStep) % 360;
            // Convert HSL to HEX
            let color: string = hslToHex(hue, saturation, lightness);
    ...
    function handle_input(new_direction : Point) {
